...
# HELPER FUNCTIONS
# ==============================================================================
def copy_cell_from_memory(src_cell_info, dst_cell):
...
async def process_panel(
    ...
    try:
        ...
        # --- THE "READ BEFORE YOU WRITE" LOGIC ---
        # Read the clean template into memory ONCE.
        template_in_memory = []
        for r_offset in range(TEMPLATE_HEIGHT):
            ...
            template_in_memory.append(row_data)
        ...
        if is_template_empty(ws, check_row=18, check_col=9):
            ...
            # Paste the pristine IN-MEMORY template data into the new blank space
            for r_offset in range(TEMPLATE_HEIGHT):
                for c_offset in range(12):
                    cell_info = template_in_memory[r_offset][c_offset]
                    dst_cell = ws.cell(row=insertion_row + r_offset, column=c_offset + 1)
                    copy_cell_from_memory(cell_info, dst_cell)
